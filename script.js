...
/* ---------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
var DB = { projects:[], floors:[], members:[], inspections:[], inspector:"" };
  ...
  if(state.tool==="select"){
    ...
    function selUp(ev){
      ...
      if(mv){
        ...
        state.selMemberId=id;                    // แค่แตะ → เลือก (คงแท็บเดิมไว้ เพื่อให้เห็นไฮไลต์ในรายการคาน)
      ...
      if(state.selMemberId){ state.selMemberId=null; render(); }   // แตะที่ว่าง = เลิกเลือก
    }
    return;
  }
...
function buildReportSheet(planCv, vm, keyCv){
  ...
  var M=Math.round(SW*0.03), brand=_cvar("brand"), ink="#12233a", sub="#5b6675";
  var p=getProject(state.projectId), f=getFloor(state.floorId), type=state.catType, sm=summarize(vm);
  // ── หัวกระดาษ ──
  var y=M;
  cx.fillStyle=brand; cx.fillRect(M, y, Math.round(SW*0.012), Math.round(SH*0.052));   // แถบสีนำหน้าชื่อ
  cx.textBaseline="top"; cx.textAlign="left";
  var tS=Math.round(SW*0.020);
  cx.fillStyle=ink; cx.font="800 "+tS+"px "+FF;
  cx.fillText("รายงานแปลนตรวจเหล็กเสริม", M+Math.round(SW*0.028), y+2);
  cx.fillStyle=sub; cx.font="500 "+Math.round(SW*0.0125)+"px "+FF;
  cx.fillText((p?p.name:"—")+"  ·  "+(f?f.name:"—")+"  ·  หมวด "+(TYPE_EN[type]||TYPES[type].label), M+Math.round(SW*0.028), y+tS+Math.round(SH*0.012));
  // วันที่ (ชิดขวา)
  cx.textAlign="right"; cx.fillStyle=sub; cx.font="500 "+Math.round(SW*0.012)+"px "+FF;
  cx.fillText("วันที่ออกรายงาน  "+dMY(Date.now()), SW-M, y+2);
  cx.textAlign="left";
  // แถวชิปสรุป
  var chipY=y+Math.round(SH*0.058), chipH=Math.round(SH*0.045), chipFS=Math.round(SW*0.0115), cxp=M;
  function chip(txt, dot){
    cx.font="700 "+chipFS+"px "+FF;
    var pad=Math.round(SW*0.012), dotR=dot?Math.round(chipFS*0.42):0, gap=dot?Math.round(chipFS*0.6):0;
    var w=pad*2+ (dot?dotR*2+gap:0) + cx.measureText(txt).width;
    cx.fillStyle="#f1f3f7"; _rr(cx,cxp,chipY,w,chipH,chipH/2); cx.fill();
    var tx=cxp+pad;
    if(dot){ cx.fillStyle=dot; cx.beginPath(); cx.arc(cxp+pad+dotR,chipY+chipH/2,dotR,0,7); cx.fill(); tx=cxp+pad+dotR*2+gap; }
    cx.fillStyle=ink; cx.textBaseline="middle"; cx.fillText(txt, tx, chipY+chipH/2+1); cx.textBaseline="top";
    cxp+=w+Math.round(SW*0.01);
  }
  chip("ทั้งหมด "+sm.total+" ชิ้น", null);
  chip("ผ่าน "+sm.pass, _cvar("pass"));
  chip("ไม่ผ่าน "+sm.fail, _cvar("fail"));
  chip("รอตรวจ "+sm.todo, _cvar("text-dim"));
  var headBottom=chipY+chipH+Math.round(SH*0.02);
  ...
  var leg=_legendForExport(vm);
  var legFS=Math.round(SW*0.011), swz=Math.round(SW*0.016), legRowH=Math.round(SH*0.036);
  var legTitleH=Math.round(SH*0.03);
  var legH = leg.length? (legTitleH + leg.length*legRowH + Math.round(SH*0.01)) : 0;
  var legTop=SH-M-legH;
  if(leg.length){
    cx.fillStyle=ink; cx.font="800 "+Math.round(SW*0.0125)+"px "+FF; cx.textBaseline="top";
    cx.fillText("ตารางสี (Legend)", M, legTop);
  ...
  var areaTop=headBottom+Math.round(SH*0.02), areaBottom=(leg.length?legTop:SH-M)-Math.round(SH*0.015);
...
function renderVisibleRegion(){
  ...
  if(!sw||!sh) return;
  if(z<=1.05){ det.style.display="none"; return; }   // ซูมต่ำ → ใช้ภาพเต็มหน้าพอ
  ...
  // supersample ×1.5 + ใช้ devicePixelRatio เต็ม → คมกว่า 1:1 พิกเซลจอ (เหมือน FOXIT)
...
function applyBestImage(){
  ...
  if(doc.kind==="img"){
    ...
    if(det){ doc.regionSig=null; }   // ไม่ซ่อนเลเยอร์คมเดิม — รอจนอันใหม่พร้อมค่อยแทนที่ (กันภาพเบลอวูบ)
    // (1) ภาพคมทั้งหน้าความละเอียดสูง — เชื่อถือได้ (กลไกเดียวกับตอนนำเข้า) คมถึง ~3.3 เท่า
    if(doc.bigUrl){ if(img.src!==doc.bigUrl) img.src=doc.bigUrl; }
    else if(!doc.bigRendering){
      doc.bigRendering=true;
      var bigTarget=(window.innerWidth||1024)<760 ? 3200 : 6000;   // ภาพฐานความละเอียดสูง → คมถึง ~5-6 เท่าโดยไม่ต้องพึ่ง region
      renderPdfToCanvas(doc.page, bigTarget).then(function(cv){
        var setu=function(u){ doc.bigRendering=false; doc.bigUrl=u;
          if(planSourceKey()===key){ var im=$(".plan-img"); if(im) im.src=u; }
          planTip("ภาพคมทั้งหน้า "+cv.width+"×"+cv.height);
      ...
      }).catch(function(e){ doc.bigRendering=false; planTip("เรนเดอร์ภาพคมไม่สำเร็จ: "+(e&&e.message||e),true); planLog("เรนเดอร์ภาพคมไม่สำเร็จ: "+(e&&e.message||e),true); });
    }
    // (2) region เสริม — คมสุดเฉพาะส่วนที่ซูมเข้า (ถ้าทำงาน)
    renderVisibleRegion();
  }
}
...
function doAuth(signup){
  ...
  fn.catch(function(e){ _authErr(_authMsg(e&&e.code)); });
