...
function navigate(screen){
  ...
  if(leaving==='memberDetail' || leaving==='memberForm' || leaving==='planEditor' || leaving==='progress'){
...
function startCloudSession(user){
  ...
  CLOUD_COLLS.forEach(function(c){
    var un=fbDb.collection(c).onSnapshot(function(snap){
      ...
      // ADD / UPDATE ทีละ item — แต่ SKIP ถ้า local มี "งานที่แก้แล้วยังไม่ sync"
      //   (ตัดสินโดยเทียบ JSON ของ item ใน DB กับที่จำไว้ใน oldBase — ถ้าไม่เท่ากัน = แก้ local แล้ว)
      snap.docs.forEach(function(d){
        ...
        // ตรวจว่า local ตัวนี้แตกต่างจาก syncBase เดิมไหม (แก้ local ยังไม่ sync)
        var localJson; try{ localJson=JSON.stringify(_forCloud(c, ex)); }catch(e){ localJson=null; }
        var lastSynced=oldBase[incoming.id];
        if(localJson!=null && lastSynced!=null && localJson!==lastSynced) return;   // เก็บ local ไว้
        // ปลอดภัย → merge in-place เพื่อรักษา object identity
