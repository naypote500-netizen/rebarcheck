...
function buildReportSheet(planCv, vm, keyCv){
  ...
  cx.fillText("รายงานแปลนตรวจเหล็กเสริม", tX, y);
  ...
  cx.fillText((p?p.name:"—")+"  ·  "+(f?f.name:"—")+"  ·  หมวด "+(TYPE_EN[type]||TYPES[type].label), tX, r2Y+r2H/2);
  cx.textBaseline="top";
  ...
  var chips=[["ทั้งหมด "+sm.total+" ชิ้น",null],["ผ่าน "+sm.pass,_cvar("pass")],["ไม่ผ่าน "+sm.fail,_cvar("fail")],["รอตรวจ "+sm.todo,_cvar("text-dim")]];
...
function renderVisibleRegion(){
  ...
  var SS=2.0, cap=(window.innerWidth||1024)<760 ? 4000 : 5200;   // เพดานคมส่วนที่เห็น (มือถือดันขึ้นจาก 3400 แต่ยังปลอดภัยกับ iOS ~16MP)
...
function applyBestImage(){
  ...
  if(doc.kind==="img"){
    ...
    if(doc.bigUrl){ if(img.src!==doc.bigUrl) img.src=doc.bigUrl; }
      ...
      var bigTarget=(window.innerWidth||1024)<760 ? 4000 : 6000;   // ภาพฐานความละเอียดสูง (มือถือดันขึ้นจาก 3200)
...
function init(){
  try{ console.log("%c[RebarCheck] เวอร์ชัน 129 โหลดแล้ว — เพิ่มความคมแปลนบนมือถือ (ดันเพดาน deep-zoom)","color:#3a5bd0;font-weight:700"); }catch(e){}
  initTheme();
  if(!CLOUD){
    loadDB();
