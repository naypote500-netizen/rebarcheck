...
document.addEventListener("click",function(e){
  ...
  switch(act){
    ...
    case "typeGoMember": {   // จากการ์ดประเภท → ไปหาชิ้นนั้น (ข้ามชั้น/แปลนได้)
      ...
      state.selMemberId=gm.id; state.memberId=gm.id; state.rightTab="props"; state.rpSheet="open";
...
function annotSvg(a, VW, VH, z, sel){
  ...
  if(a.kind==="dim"){
    ...
    var lbl=(a.label||""), tw=Math.max(18, lbl.length*fs*0.62), gap=tw/2+6/z;
...
var RV_TABS=[["file","ไฟล์"],["structure","โครงสร้าง"],["draw","วาด"],["annot","หมายเหตุ"],["view","มุมมอง"],["progress","เทคอนกรีต"]];
function rvRibbonHtml(type, f, plan, plans, selM){
  ...
  if(rt==="file"){
    ...
    body+=rvGrp('แก้ไข', rvCol(rvSm(false,"annotDup",'','copy','ทำซ้ำ','ทำซ้ำหมายเหตุที่เลือก',!selA)+rvSm(false,"annotDel",'','trash','ลบ','ลบหมายเหตุที่เลือก  (Delete)',!selA)));
    body+=rvGrp('รูปแบบ', rvCol(rvSm(!!state.selAnnotId,"annotPanel",'','palette','แผงรูปแบบ','เปิดแผงปรับฟอนต์/สี/หัวลูกศร',!selA)+rvSm(false,"planUndo",'','undo','ย้อนกลับ','Ctrl+Z',!(PL_UNDO[state.floorId]||[]).length)));
  }else if(rt==="progress"){
    body+=rvGrp('เลือก', rvBig(state.tool==="select","setTool",'data-tool="select"','sel','เลือก/ย้าย','เลือก/ย้าย  (V)'));
    ...
    var selT=selM?memberTypeOf(selM):null;
    body+=rvGrp('ประเภท (รายละเอียดร่วม)', rvBig(false,"typeCreate",'','link','สร้างประเภท','สร้างประเภทจากชิ้นที่เลือก — แผ่นรายละเอียดของชิ้นนี้จะกลายเป็นของประเภท ให้ชิ้นอื่นใช้ร่วมได้',!hasSel||!!selT)
      +rvCol(rvSm(false,"typeAssignSheet",'','wand','กำหนดประเภท','เลือกประเภทให้ชิ้นที่เลือก',!hasSel)+rvSm(false,"typeDetach",'','unlink','แยกออก','ถอดชิ้นนี้ออกจากประเภท',!selT)));
...
function rvPaletteHtml(type, m){
  ...
  if(view==="inspect"){
    ...
    h+='<div class="rv-prow"><span>ลงสีตาม</span><b><span class="rv-seg"><button data-act="colorMode" data-mode="status" aria-pressed="'+(state.colorMode==="status")+'">สถานะ</button><button data-act="colorMode" data-mode="plain" aria-pressed="'+(state.colorMode==="plain")+'">ที่ตั้งเอง</button><button data-act="colorMode" data-mode="type" aria-pressed="'+(state.colorMode==="type")+'">ประเภท</button></span></b></div>';
...
function rvBrowserHtml(type, p, f, plans, plan){
  ...
  // ประเภทชิ้นส่วน (แผ่นรายละเอียดใช้ร่วมกัน) — ทั้งโครงการ
  var tps=typesOf(state.projectId);
  h+='<div class="t2 hd"><span>ประเภทชิ้นส่วน ('+tps.length+')</span><button class="rv-tbtn" data-act="typeCreate" title="สร้างประเภทจากชิ้นที่เลือก">'+rvIc('plus',12)+'</button></div>';
  TYPE_ORDER.forEach(function(t){
    var tl=tps.filter(function(x){ return x.mtype===t; }); if(!tl.length) return;
    h+='<div class="t3 grp"><span class="rv-tsw" style="background:var(--t-'+TYPES[t].css+')"></span><span>'+esc(TYPES[t].label)+'</span><em>'+tl.length+' ประเภท</em></div>';
    tl.forEach(function(x){
      h+='<button class="t4'+(!state.selMemberId&&x.id===state.selTypeId?" on":"")+'" data-act="typeSelect" data-tid="'+esc(x.id)+'" title="'+esc(x.name)+'"><span class="rv-tsw" style="background:'+esc(x.color||"#2563eb")+'"></span><span class="mono">'+esc(x.name)+'</span><em>'+typeMembers(x.id).length+'</em></button>';
    });
  });
  if(!tps.length) h+='<div class="t3 muted">ยังไม่มี — เลือกชิ้นแล้วกด “สร้างประเภท”</div>';
  // โซนเท
...
function annotFormatHtml(){
  ...
  h+='<div class="fx-r"><textarea class="fx-in fx-ta" id="annotText" rows="'+(isDim?1:3)+'" placeholder="'+(isDim?'เช่น 3.350':'พิมพ์ข้อความ…')+'">'+_hx(isDim?(a.label||""):(a.text||""))+'</textarea></div>';
...
function bindPlanEditor(){
  ...
  _plKeyHandler=function(e){
    ...
    if(k==="Escape"){
      ...
      if(state.selMemberId||state.selZoneId||state.selAnnotId){ state.selMemberId=null; state.selZoneId=null; state.selAnnotId=null; render(); }
      return;
    }
    ...
    else if(kl==="v"){ state.tool="select"; render(); }
  ...
  if(state.tool==="select"){
    var ps=null, moved=false, mid=null, tf=null, mv=null, ml=null, lsz=null, ztf=null, zmv=null, atf=null, amv=null;
    ...
    function selUp(ev){
      ...
      if(atf){ atf=null; plCommitPend(); saveDB(); return; }
      if(amv){
        var wasA=amv.moved, aid=amv.aid; amv=null;
        if(wasA){ plCommitPend(); saveDB(); return; }
        state.selAnnotId=aid; state.selMemberId=null; state.selZoneId=null; state.ribbonTab="annot"; render();   // แค่แตะ → เลือก + เปิดแผงรูปแบบ
        return;
      }
      ...
      if(state.selMemberId||state.selZoneId||state.selAnnotId){ state.selMemberId=null; state.selZoneId=null; state.selAnnotId=null; render(); }   // แตะที่ว่าง = เลิกเลือก
    }
    return;
  }
...
function finishDrawDim(s, p){
  ...
  var suggest = us>0 ? (pxLen*us).toFixed(3) : "";
  var lbl=window.prompt(us>0 ? "ระยะที่วัดได้ (แก้ได้):" : "ใส่ระยะจริงของเส้นนี้ (เช่น 3.350)\nเส้นแรกที่ใส่ตัวเลขจะใช้เป็นมาตราส่วนของแปลนนี้:", suggest);
  ...
  var a={ id:uid("an"), projectId:state.projectId, floorId:state.floorId, planId:pid, kind:"dim",
    p1:{x:s.x,y:s.y}, p2:{x:p.x,y:p.y}, label:String(lbl).trim(),
