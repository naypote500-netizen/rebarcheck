...
function bindPlanEditor(){
  ...
  if(state.tool==="select"){
    ...
    function selUp(ev){
      ...
      if(atf){ var _atA=getAnnot(atf.aid); atf=null; plCommitPend(); saveDB(); if(_atA && (_atA.kind==="area" || (_atA.kind==="dim"&&_atA.auto))) render(); return; }   // ตัวเลขในแผงต้องตามรูปที่ลาก
      if(amv){
