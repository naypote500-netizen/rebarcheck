...
function renderVisibleRegion(){
  ...
  var SS=_mob?3.0:2.0, cap=_mob?4200:5200;   // มือถือ: supersample สูงขึ้น (คมกว่าจอ retina หลายเท่า) เพดาน 4200 ยังไม่เกินลิมิต iOS (~16MP)
...
function applyBestImage(){
  ...
  if(doc.kind==="img"){
    ...
    if(doc.bigUrl){ if(img.src!==doc.bigUrl) img.src=doc.bigUrl; }
      ...
      var bigTarget=(window.innerWidth||1024)<760 ? 4500 : 6000;   // ภาพฐานความละเอียดสูง (มือถือ 4500 — เผื่อตอนซูมกลางๆ ก่อน deep-zoom จะทัน)
...
function init(){
  try{ console.log("%c[RebarCheck] เวอร์ชัน 131 โหลดแล้ว — ดันความคมแปลนบนมือถือ (supersample x3 + ภาพฐาน 4500)","color:#3a5bd0;font-weight:700"); }catch(e){}
  initTheme();
  if(!CLOUD){
    loadDB();
