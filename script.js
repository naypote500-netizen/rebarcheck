...
function viewHome(){
  ...
  var side='<aside class="dh-side">'
    ...
    +'<span class="dh-nm">Rebar<b>Check</b><small>ตรวจเหล็กเสริมก่อนเท</small></span></div>'
    +'<nav class="dh-navs">'
    + homeNav("goHome",true,"หน้าหลัก",IC.home)
    + homeNav("goProjects",false,"ตรวจสอบชิ้นส่วน",IC.search)
    + homeNav("goProgress",false,"เทคอนกรีต",IC.progress)
    ...
    +'<div class="bs">ตรวจให้ชัวร์<br>ก่อนเทคอนกรีต</div><div class="ver">RebarCheck v1.0.0</div></div></aside>';
  // ---- topbar ----
  ...
  var hero='<section class="dh-hero has-photo">'
    +'<img class="dh-hero-bg" src="img/hero-site.webp?v=2" alt="" decoding="async" data-c="has-photo" onerror="this.parentNode.classList.remove(this.dataset.c);this.remove()">'
...
function planCtxMenu(m, x, y){
  ...
  var t=memberTypeOf(m), ids=selIds(), multi=(ids.length>1 && ids.indexOf(m.id)>=0), h;
  if(multi){   // คลิกขวาบนชุดที่เลือก → ทุกคำสั่งทำกับทั้งชุด (เหมือนปุ่ม Delete) ไม่ใช่แค่ชิ้นที่คลิก
    var allH=ids.every(function(q){ var mm=getMember(q); return mm&&mm.hidden; });
    h='<div class="rv-mh mono">เลือก '+ids.length+' ชิ้น</div>'
      +it("toggleHideSel",'',allH?'eyeOff':'eye',allH?'แสดงทั้งชุด':'ซ่อนทั้งชุด')
      +it("dupSelected",'','copy','ทำซ้ำทั้งชุด')
      +it("typeAssignSheet",'','link','กำหนดประเภททั้งชุด')
      +'<div class="rv-msep"></div>'
      +it("zoomToSel",'','fit','ซูมไปหา')
      +it("clearSel",'','cross','เลิกเลือก  (Esc)')
      +'<div class="rv-msep"></div>'
      +it("delSelected",'','trash','ลบทั้งชุด  (Delete)','danger');
  }else{   // ลำดับเดียวกับกลุ่มในแท็บ "แก้ไข": ตรวจสอบ · ชิ้นส่วน · ประเภท · การเลือก · ลบ
    h='<div class="rv-mh mono">'+esc(m.code)+' · '+esc(TYPES[m.type].label)+'</div>'
      +it("goInspect",'','check','ตรวจเหล็ก  (ดับเบิลคลิก)')
      +it("showDetails",'','sheet','รายละเอียด')
      +'<div class="rv-msep"></div>'
      +it("editMember",'','edit','แก้สเปก')
      +it("assignSheet",'','template','เทมเพลตเหล็ก')
      ...
      +it("clearSel",'','cross','เลิกเลือก  (Esc)')
      +'<div class="rv-msep"></div>'
      +it("delMember",'','trash','ลบชิ้นนี้  (Delete)','danger');
  }
...
function viewMembers(){
  ...
  if(all.length===0){
    h+=emptyBox('<svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21.3 15.3a2.4 2.4 0 0 1 0 3.4l-2.6 2.6a2.4 2.4 0 0 1-3.4 0L2.7 8.7a2.4 2.4 0 0 1 0-3.4l2.6-2.6a2.4 2.4 0 0 1 3.4 0Z"/><path d="m14.5 12.5 2-2M11.5 9.5l2-2M8.5 6.5l2-2M17.5 15.5l2-2"/></svg>',"ยังไม่มีชิ้นส่วนในชั้นนี้","กด “เพิ่มชิ้นส่วน” แล้วเลือกชนิด เช่น เสา คาน พื้น RC");
  }else if(list.length===0){
    h+=emptyBox('<svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>',"ไม่พบชิ้นส่วนที่ค้นหา","ลองเปลี่ยนคำค้นหรือเลือกชนิดอื่น");
  }else{
...
function inspectionBlockHtml(m){
  ...
  h+='<div class="card"><div class="card-h"><svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3Z"/><circle cx="12" cy="13" r="3.2"/></svg> หลักฐานและการยืนยัน</div><div class="card-b">'
    +'<div class="grid2">'
    +  '<label class="file-label"><svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3Z"/><circle cx="12" cy="13" r="3.2"/></svg> ถ่ายรูป'
    +    '<input type="file" id="photoInput" accept="image/*" capture="environment"></label>'
    +  '<label class="file-label"><svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="m3 15 5-5 4 4 3-3 6 6"/><circle cx="9" cy="9" r="1.6"/></svg> เลือกจากอัลบั้ม'
    +    '<input type="file" id="photoInput2" accept="image/*" multiple></label>'
    +'</div><div class="small muted" style="margin:4px 0 0">ย่อขนาดให้อัตโนมัติ · แนบได้หลายรูป</div>'
...
function bindInspectionInputs(){
  ...
  var pi2=$("#photoInput2"); if(pi2) pi2.addEventListener("change",onPhotoPick);
...
/** สำเนาชิ้นส่วนสำหรับ "ทำซ้ำ": id ใหม่ · ขยับตำแหน่งเล็กน้อยไม่ให้ทับตัวเดิม · แผ่นรายละเอียด: ผูกประเภท=ใช้ร่วม, ของตัวเอง=ก็อป (ยังไม่ push ลง DB) */
function cloneMemberForDup(src){
  var cp=JSON.parse(JSON.stringify(src));
  cp.id=uid("m"); cp.hidden=false;
  if(cp.typeId) cp.doc={els:[]};
  else if(cp.doc && Array.isArray(cp.doc.els)) cp.doc={els:deCloneEls(cp.doc.els), page:cp.doc.page};
  if(cp.plan){
    var off=0.03;
...
function confirmInspection(){
  ...
  if(state.screen==="planEditor"){ state.rightTab="props"; render(); }   // กลับแผงข้อมูลชิ้น (มี "ผลตรวจล่าสุด") — ไม่ค้างเช็คลิสต์ว่างให้เข้าใจผิดว่ายังไม่บันทึก   // อยู่ในเอดิเตอร์: รีเฟรชให้จุดสถานะบนแปลนเปลี่ยน
...
function applyTplToMember(m, d){
  ...
  if(m.plan){ if(d.fill!=null)m.plan.fill=d.fill; if(d.fillA!=null)m.plan.fillA=d.fillA; if(d.strokeW!=null)m.plan.strokeW=d.strokeW;
    if(isBox(m.plan) && (d.fill!=null||d.fillA!=null)) setGroupFill(m, d.fill||null, d.fillA);
    if(isBox(m.plan) && d.strokeW!=null) setGroupStroke(m, d.strokeW); }
...
function saveMemberForm(andNew){
  ...
  if(!editing){
    ...
    DB.members[i]=m;
    if(m.code!==editing.code || m.type!==editing.type) adoptGroupStyle(m);   // เปลี่ยนเบอร์ → สีตามเบอร์ใหม่
...
function viewData(){
  ...
  return '<div class="screen-title">ข้อมูล / สำรอง</div>'
    ...
    +   '<hr class="sep">'
    +   '<p class="small muted" style="margin-top:0">ส่งออกไฟล์สำรองเก็บไว้ หรือส่งให้เครื่องอื่นนำเข้าได้</p>'
    +   '<div class="grid2">'
    +     '<button class="btn block" data-act="export"><svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12M7 10l5 5 5-5M5 21h14"/></svg> ส่งออกไฟล์สำรอง (.json)</button>'
    +     '<label class="btn ghost block" style="text-align:center;margin:0" for="fileImport"><svg class="ic" viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 21V9M7 14l5-5 5 5M5 3h14"/></svg> นำเข้าไฟล์สำรอง</label>'
    +   '</div>'
...
var state = {
  ...
  styleTab2:"fill",      // พาเนลชิ้นที่เลือก (คอม): แท็บ สีพื้น | เส้นกรอบ
  ...
  strokeW:0,             // ความหนาเส้นกรอบ (0 = ไม่มีเส้น) — วาดใหม่ไม่มีเส้นขอบ ปรับได้ที่แผง "สีกรอบ"
...
document.addEventListener("click",function(e){
  ...
  switch(act){
    ...
    case "goProjects": {   // เข้าสู่การตรวจ → โครงการที่เปิดอยู่ (หรือล่าสุด) · ยังไม่มีโครงการ → เปิดฟอร์มสร้าง
      ...
      if(_gp) go("floors",{projectId:_gp.id, floorId:null, q:"", typeFilter:"all"}); else dlgProject(null);
      break;
    }
    ...
    case "tabInspect":  { var _pj=getProject(state.projectId)||(DB.projects||[]).slice().sort(function(a,b){ return (b.createdAt||0)-(a.createdAt||0); })[0]; if(_pj) go("floors",{projectId:_pj.id, floorId:null, q:"", typeFilter:"all"}); else navigate("home"); break; }
    case "addFloorFab": dlgFloor(null); break;
    case "goData":      navigate("data"); break;
    case "goReports": {   // รายงานของโครงการที่เปิดอยู่ (อยู่ในชั้น → กรองชั้นนั้นให้เลย)
      var _rp=getProject(state.projectId)||(DB.projects||[]).slice().sort(function(a,b){ return (b.createdAt||0)-(a.createdAt||0); })[0];
      var _rf=(["planEditor","members","memberDetail","floorPlans"].indexOf(state.screen)>=0)?getFloor(state.floorId):null;
    ...
    case "setStyleTab2": state.styleTab2=el.getAttribute("data-stab")||"fill"; render(); break;
    ...
    case "warnUnplaced": {   // ชิ้นที่ยังไม่วางบนแปลน → เปิดแท็บผัง + เลือกชิ้นแรก + บอกวิธีวาง
      ...
      state.selMemberId=_un[0].id; state.memberId=_un[0].id; state.selZoneId=null; state.selAnnotId=null; if(!isMobile()) _toModifyTab();
    ...
    case "drawTcallStart":   state.tool="drawTcall"; state.selAnnotId=null; state.selMemberId=null; state.selMulti=[]; state.selZoneId=null; state.marquee=false; render(); break;
    case "tboxArrow": { var _tw=getAnnot(state.selAnnotId); if(!_tw||_tw.kind!=="tbox") break; plPushUndo(); _tw.a1=el.getAttribute("data-v")||"open"; annotRemember(_tw); saveDB(); render(); break; }
    case "tboxAddTip": { var _tq=getAnnot(state.selAnnotId); if(!_tq||_tq.kind!=="tbox") break; plPushUndo();
      if(_tq.tip) delete _tq.tip;
      else { var _bx1=Math.min(_tq.box.x1,_tq.box.x2), _by2=Math.max(_tq.box.y1,_tq.box.y2); _tq.tip={x:Math.max(0,_bx1-0.05), y:Math.min(1,_by2+0.06)}; }
      saveDB(); render(); break; }
    ...
    case "applyStyleAll": {
      ...
      _list.forEach(function(mm){ setGroupFill(mm, state.fillColor, state.fillAlpha); setGroupStroke(mm, state.strokeW); });   // เบอร์/ประเภทเดียวกันชั้นอื่นตาม
      saveDB();
    ...
    case "selectPlanMember":
      state.selMemberId=id; state.rightTab="inspect"; state.answers={}; state.photos=[]; state.note=""; if(!isMobile()) _toModifyTab();
      render();
      break;
    case "toggleHide": {
      var hm=getMember(id);
    ...
    case "dupSelected": {   // ทำซ้ำทั้งชุดที่เลือก → เลือกชุดสำเนาให้ลากย้ายต่อได้ทันที
      var _ds=selIds().map(getMember).filter(Boolean); if(!_ds.length) break;
      plPushUndo();
      var _new=_ds.map(cloneMemberForDup); _new.forEach(function(c){ DB.members.push(c); });
      if(!saveDB()){ DB.members=DB.members.filter(function(x){ return _new.indexOf(x)<0; }); render(); break; }
      setMulti(_new.map(function(c){ return c.id; })); render(); toast("ก๊อป "+_new.length+" ชิ้นแล้ว — ลากย้ายทั้งชุดได้เลย");
      break;
    }
    case "editListMember": {
      var lm=getMember(id);
    ...
    case "typeAssignSheet": {
      ...
      openSheet('<h3>กำหนดประเภทให้ '+(selIds().length>1?selIds().length+' ชิ้นที่เลือก':esc(am.code))+'</h3><p class="small muted" style="margin-top:0">ชิ้นที่ใช้ประเภทเดียวกันจะใช้หน้ารายละเอียด (รูป/ข้อความ/ตาราง) เดียวกัน — แก้ที่ชิ้นไหนก็เปลี่ยนทุกชิ้น</p>'+hb0);
      break;
    }
    case "typeAssignTo": { var am2=getMember(state.selMemberId), tt=getType(el.getAttribute("data-tid")); closeSheet(); if(!am2||!tt) break;
      var _tids=selIds(); if(_tids.length>1) typeAssignMany(_tids, tt); else typeAssign(am2, tt); break; }
    case "typeSelect": {
    ...
    case "typeGoMember": {   // จากการ์ดประเภท → ไปหาชิ้นนั้น (ข้ามชั้น/แปลนได้)
      ...
      state.selMemberId=gm.id; state.memberId=gm.id; state.rightTab="props"; state.rpSheet="open"; if(!isMobile()) _toModifyTab();
    ...
    case "renameMember": {
      ...
      plPushUndo(); rm.code=nc; adoptGroupStyle(rm); saveDB(); render(); toast("เปลี่ยนเป็น “"+nc+"”");
      break;
    }
    ...
    case "assignToSel": {   // Assign เทมเพลตให้ชิ้นส่วนที่เลือกบนแปลน (ไฮไลท์แล้ว Assign)
      ...
      if(sm2 && tp2){
        ...
        applyTplToMember(sm2, tp2.data); saveDB(); closeSheet(); render(); toast("ใส่ค่าจากเทมเพลต “"+tp2.name+"” ให้ "+sm2.code+" แล้ว");
      }
      break;
    }
    case "assignSheet": {   // เปิดรายการเทมเพลตเพื่อ Assign ให้ชิ้นส่วนที่เลือก (เฉพาะประเภทเดียวกัน)
      ...
      openSheet('<h3>ใส่ค่าจากเทมเพลตให้ '+esc(sm3.code)+'</h3>'+hb);
      break;
    }
...
var TBOX_DEF={ color:"#1d4ed8", bcol:"#3b5bdb", fill:"#dbe4ff", fillA:1, sw:1.5, radius:0, font:"Sarabun", size:14,
                bold:0, italic:0, underline:0, strike:0, align:"center", op:1, a1:"open" };
...
function tboxSvg(a, VW, VH, z, sel){
  ...
  if(a.tip){   // ป้ายชี้: เส้นจากปลายลูกศร → ข้อศอก → กลางขอบกล่องฝั่งที่ใกล้
    var tpx=a.tip.x*VW, tpy=a.tip.y*VH, cyb=by+bh/2, lft=(tpx<bx+bw/2);
    var axp=lft?bx:bx+bw, kx=axp+(lft?-1:1)*Math.min(14*k, Math.max(0, Math.abs(tpx-axp)*0.5));
    var lw=Math.max(0.8, (+s.sw>0?+s.sw:1.2))*k, lc=_hx(+s.sw>0?s.bcol:s.color);
    var dpath='M'+tpx+' '+tpy+' L'+kx+' '+cyb+' L'+axp+' '+cyb;
    out+='<path d="'+dpath+'" fill="none" stroke="transparent" stroke-width="'+(12/z)+'"/>';   // ที่กดง่าย
    out+='<path d="'+dpath+'" fill="none" stroke="'+lc+'" stroke-width="'+lw+'" stroke-linejoin="round"/>';
    var ang=Math.atan2(tpy-cyb, tpx-kx), L=Math.max(5*k, lw*4.5);
    if(Math.abs(tpx-kx)<1e-6 && Math.abs(tpy-cyb)<1e-6) ang=0;
    var hp=function(d){ return [tpx-L*Math.cos(ang+d), tpy-L*Math.sin(ang+d)]; }, p1=hp(0.45), p2=hp(-0.45);
    if(s.a1==="solid") out+='<path d="M'+tpx+' '+tpy+' L'+p1[0]+' '+p1[1]+' L'+p2[0]+' '+p2[1]+' Z" fill="'+lc+'"/>';
    else if(s.a1==="dot") out+='<circle cx="'+tpx+'" cy="'+tpy+'" r="'+(L*0.38)+'" fill="'+lc+'"/>';
    else if(s.a1!=="none") out+='<path d="M'+p1[0]+' '+p1[1]+' L'+tpx+' '+tpy+' L'+p2[0]+' '+p2[1]+'" fill="none" stroke="'+lc+'" stroke-width="'+lw+'" stroke-linecap="round" stroke-linejoin="round"/>';
  }
  ...
  if(sel){
    ...
    if(a.tip) out+='<circle data-aid="'+a.id+'" data-ahandle="tip" cx="'+(a.tip.x*VW)+'" cy="'+(a.tip.y*VH)+'" r="'+(5.2/z)+'" fill="#fff" stroke="#2b6fe0" stroke-width="'+(2/z)+'" style="cursor:crosshair"><title>ลากเพื่อย้ายปลายลูกศร</title></circle>';
  }
  return out;
}
...
function planShapesSVG(VW,VH){
  ...
  members.forEach(function(m){
    ...
    if(pl.kind==="line"){
      ...
      var scol=(state.colorMode==="status") ? fcol : (pl.strokeCol||fcol);   // สีเส้นกรอบแยกได้ (ไม่ตั้ง = ตามสีพื้น)
      var strokeAttr = bw>0 ? ' stroke="'+scol+'" stroke-width="'+bw+'"' : ' stroke="none"';
      var mainEl;
      if(pl.kind==="oval"){
...
function plRestore(j){
  ...
  var _rgSeen={};
  (snap.m||[]).forEach(function(w){   // สีที่คืนค่า → เบอร์/ประเภทเดียวกัน (ทุกชั้น) ตามด้วย (กันสีไม่ตรงกันตอนเปิดแอป)
    if(!w.plan || !isBox(w.plan)) return;
    var lm=getMember(w.id); if(!lm) return;
    var gk=lm.typeId?("t:"+lm.typeId):("c:"+_codeKey(lm)); if(_rgSeen[gk]) return; _rgSeen[gk]=1;
    setGroupFill(lm, lm.plan.fill||null, (lm.plan.fillA!=null)?lm.plan.fillA:null);
    if(lm.plan.strokeW!=null) setGroupStroke(lm, lm.plan.strokeW, lm.plan.strokeCol||"");
  });
...
function legendHtml(members){
  ...
  if(state.colorMode==="status"){   // โหมดสีตามสถานะตรวจ → ตารางสี = ผ่าน/ไม่ผ่าน/รอตรวจ (สีเดียวกับที่วาด)
    var cs={pass:0,fail:0,todo:0}; members.forEach(function(m){ if(m.plan) cs[memberStatus(m)]=(cs[memberStatus(m)]||0)+1; });
    var rowsS=[["pass","var(--pass)","ผ่าน"],["fail","var(--fail)","ไม่ผ่าน"],["todo","var(--text-dim)","รอตรวจ"]].map(function(r){ return '<div class="lg-row"><span class="lg-sw" style="background:'+r[1]+'"></span><span class="lg-tx">'+r[2]+' ('+(cs[r[0]]||0)+')</span></div>'; }).join("");
    return '<div class="plan-legend" id="planLegend"><div class="lg-head">สถานะตรวจ</div><div class="lg-body">'+rowsS+'</div></div>';
  }
  var groups={}, order=[];
  members.forEach(function(m){
    if(!m.plan) return;
...
var DE_STAMPS={pass:["✓ ผ่าน","#1a8a56"], fix:["✗ แก้ไข","#d23b46"], recheck:["! รอตรวจซ้ำ","#b07d1a"], name:["","#1d4ed8"], date:["","#1d4ed8"], code:["","#111827"]};
...
function deLibFlyHtml(){
  ...
  return '<div class="de-libfly"><div class="de-flyhead">'+tabs+'<span class="sp"></span><button class="fx-x" data-de="shapes" title="ปิด">'+rvIc('cross',12)+'</button></div><div class="de-libgrid">'+items+'</div></div>';
}
...
function deRibbonHtml(m,t){
  ...
  return '<div class="rv-tabs">'+tabs+'<span class="sp"></span><span class="rv-mode">'+(t?''+rvIc('link',11)+' ประเภท '+esc(t.name)+' · ใช้ร่วม '+typeMembers(t.id).length+' ชิ้น':'รายละเอียดเฉพาะชิ้นนี้')+'</span></div><div class="rv-ribbon">'+body+'</div>';
}
...
function dePaletteHtml(m,t){
  ...
  if(t){
    ...
    h+='<div class="rv-ph" style="background:#e7f0fd;color:#1b4f9c">'+rvIc('link',11)+' แผ่นนี้ใช้ร่วมกัน '+ms.length+' ชิ้น</div>';
    h+='<div class="rv-pnote"><span class="mono">'+ms.map(function(x){ return esc(x.code); }).join(' · ')+'</span><br>แก้ที่นี่ = เปลี่ยนทุกชิ้น</div>';
    h+='<div class="rv-pacts"><button class="btn soft" data-act="typeManage">'+rvIc('link',14)+' จัดการประเภท</button><button class="btn soft" data-act="typeDetach">'+rvIc('unlink',14)+' แยกออก</button></div>';
  }else{
...
function deStatusHtml(m,t){
  ...
  return '<div class="rv-status"><span class="rv-smsg">'+esc(msg)+'</span><span class="sp"></span>'
    ...
    +'<span class="rv-ssep"></span><span class="rv-zoom"><button data-de="zoomout" title="ซูมออก (−)">−</button><span id="deZoomLabel">'+Math.round(deZ()*100)+'%</span><button data-de="zoomin" title="ซูมเข้า (+)">+</button><button data-de="zoomfit" title="พอดีจอ (0)">'+rvIc('fit',12)+'</button></span></div>';
}
function deQatHtml(p,f,m,t){
  ...
  return '<div class="rv-qat">'
    ...
    +'<span class="rv-qtitle">RebarCheck — '+esc(p?p.name:"")+' · '+esc(f?f.name:"")+' · รายละเอียด '+esc(m.code)+(t?' · '+rvIc('link',11)+' '+esc(t.name):'')+'</span><span class="sp"></span>'
    +'<button class="rv-qb" data-de="exportpdf" title="ออก PDF">'+rvIc('pdf',14)+'</button></div>';
}
...
var RV_IC={
  ...
  csv:'<rect x="3" y="3" width="18" height="12" rx="2"/><path d="M3 9h18M9 3v12M12 22v-5M9.5 19.5 12 22l2.5-2.5"/>',
  warn:'<path d="M12 3 2 21h20L12 3Z"/><path d="M12 10v5M12 18h.01"/>',
  sheet:'<path d="M14 3v5h5"/><path d="M9 4H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8l-5-5H9"/><path d="M8 13h8M8 17h5"/>',
...
/** เปิดแท็บบริบท "แก้ไข" — จำแท็บที่ใช้อยู่ไว้ให้กลับเมื่อเลิกเลือก */
function _toModifyTab(){ if(state.ribbonTab!=='modify') state.prevRibbonTab=state.ribbonTab; state.ribbonTab='modify'; }
var RV_TABS=[["structure","โครงสร้าง"],["annot","หมายเหตุ"],["progress","เทคอนกรีต"],["view","มุมมอง"]];
var RV_TAB_TIP={structure:"วาด · เลือก · แก้ไขชิ้นส่วนเหล็ก", annot:"กล่องข้อความ · เส้นบอกขนาด · มาตราส่วน · วัดพื้นที่", progress:"วาดโซนเทและอัพเดทสถานะการเทคอนกรีต", view:"ลงสี · ป้ายเบอร์ · พาเนล"};
...
function rvFileMenuHtml(f, plan, plans){
  ...
  h+=it("exportProgressPdf",'','pdf','ออก PDF — เทคอนกรีต');
  h+=it("areaCsv",'','csv','ส่งออก CSV — ตารางปริมาณพื้นที่');
  return h+'</div>';
}
function rvRibbonHtml(type, f, plan, plans, selM){
  ...
  if(rt==="modify" && nSel>1){   // เลือกหลายชิ้น
    ...
    var selT=memberTypeOf(selM);
    body+=selBtn;
    body+=rvGrp('ตรวจสอบ', rvBig(state.rightTab==="inspect","goInspect",'','check','ตรวจเหล็ก','เช็คลิสต์ตรวจเหล็กก่อนเท')+rvBig(false,"showDetails",'','sheet','รายละเอียด','ชีตรายละเอียด (รูป/ข้อความ/ตาราง)'));
    body+=rvGrp('ชิ้นส่วน', rvCol2(
       rvSm(false,"editMember",'','edit','แก้สเปก','แก้ไขข้อมูลเหล็ก')
      +rvSm(false,"assignSheet",'','template','เทมเพลตเหล็ก','ใส่ข้อมูลเหล็กจากเทมเพลตที่บันทึกไว้')
      ...
      +rvCol(rvSm(false,"delMember",'','trash','ลบ','ลบชิ้นนี้  (Delete)')));
    body+=rvGrp(selT?'ประเภท · '+selT.name:'ประเภท (แผ่นรายละเอียดร่วม)',
       (selT ? rvBig(false,"typeSelect",'data-tid="'+esc(selT.id)+'"','link','จัดการประเภท','ประเภท “'+selT.name+'” — ใช้ร่วมกัน '+typeMembers(selT.id).length+' ชิ้น · กดเพื่อจัดการ')
             : rvBig(false,"typeCreate",'','link','สร้างประเภท','สร้างประเภทจากชิ้นนี้ — แผ่นรายละเอียดจะใช้ร่วมกับชิ้นอื่นได้'))
      +rvCol(rvSm(false,"typeAssignSheet",'','link',selT?'เปลี่ยนประเภท':'กำหนดประเภท','เลือกประเภทให้ชิ้นนี้')
        +(selT ? rvSm(false,"typeDetach",'','unlink','แยกออก','ถอดชิ้นนี้ออกจากประเภท') : '')));
    ...
    body+=rvGrp('นำออก', rvBig(false,"exportProgressPdf",'','pdf','PDF เทคอนกรีต','ออก PDF ความคืบหน้าการเทคอนกรีตของแปลนนี้'));
  }else if(rt==="view"){
  ...
  var tabs='<button class="rv-tab rv-ftab'+(state.fileMenu?" on":"")+'" data-act="fileMenu" title="แปลน · นำออก · ข้อมูล">'+rvIc('folder',13)+'ไฟล์ '+rvIc('chev',11)+'</button>'
    ...
    +RV_TABS.map(function(t){ return '<button class="rv-tab'+(rt===t[0]?" on":"")+(t[0]==="progress"?" prog":"")+'" data-act="setRibbonTab" data-rtab="'+t[0]+'" title="'+esc(RV_TAB_TIP[t[0]]||t[1])+'">'+esc(t[1])+'</button>'; }).join("")
...
/** แบบแท็บ สีพื้น | เส้นกรอบ (แนว Foxit) — พาเนลคอมตอนเลือกชิ้นเดี่ยว */
function rvStyleTabRows(m){
  var pl=m.plan, tab=(state.styleTab2==="stroke")?"stroke":"fill";
  var fc=(pl.fill||"#f59e0b"), fa=(pl.fillA!=null?pl.fillA:0.28), sww=Math.round(pl.strokeW!=null?pl.strokeW:10), sc=pl.strokeCol||"";
  var h='<div class="rv-stabs"><button class="'+(tab==="fill"?"on":"")+'" data-act="setStyleTab2" data-stab="fill">สีพื้น</button><button class="'+(tab==="stroke"?"on":"")+'" data-act="setStyleTab2" data-stab="stroke">เส้นกรอบ</button></div>';
  if(tab==="fill"){
    h+='<div class="rv-cirwrap">'+PRESET_COLORS.map(function(c){ return '<button class="swatch cir2'+(fc.toLowerCase()===c?" on":"")+'" data-swatch="'+c+'" title="'+c+'" style="background:'+c+'"></button>'; }).join("")
      +'<span class="cir2 rainbow" title="เลือกสีเอง"><input type="color" id="drawColor" value="'+esc(fc)+'"></span></div>';
    h+='<div class="rv-prow"><span>โค้ดสี</span><b><span id="drawSwatch" class="rv-swpv" style="background:'+esc(fc)+';opacity:'+fa+'"></span><span class="mono">'+esc(fc)+'</span></b></div>';
    h+='<div class="rv-prow"><span>ความเข้ม</span><b class="rv-range"><input type="range" id="drawAlpha" min="0" max="100" value="'+Math.round(fa*100)+'"><input type="number" class="rv-num" id="alphaNum" min="0" max="100" value="'+Math.round(fa*100)+'"><span class="rv-unit">%</span></b></div>';
  }else{
    h+='<div class="rv-cirwrap"><button class="rv-autobtn'+(sc?"":" on")+'" data-strokecol="" title="ใช้สีเดียวกับสีพื้น">ตามสีพื้น</button>'
      +PRESET_COLORS.map(function(c){ return '<button class="cir2'+(sc.toLowerCase()===c?" on":"")+'" data-strokecol="'+c+'" title="'+c+'" style="background:'+c+'"></button>'; }).join("")
      +'<span class="cir2 rainbow" title="เลือกสีเอง"><input type="color" id="drawStrokeCol" value="'+esc(sc||fc)+'"></span></div>';
    h+='<div class="rv-prow"><span>โค้ดสี</span><b><span class="mono">'+(sc?esc(sc):"= สีพื้น")+'</span></b></div>';
    h+='<div class="rv-prow"><span>ความหนา</span><b class="rv-range"><input type="range" id="drawStroke" min="0" max="30" value="'+sww+'"><input type="number" class="rv-num" id="strokeNum" min="0" max="30" value="'+sww+'"><span class="rv-unit">px</span></b></div>';
  }
  return h;
}
function rvPaletteHtml(type, m, p, f, plans, plan){
  ...
  if(view==="tree"){
    ...
    if(z){
      ...
      h+='<div class="rv-pacts"><button class="btn soft" data-act="manageZoneStatus">'+rvIc('gear',14)+' จัดการสถานะ + สี</button></div>';
    }
    ...
    if(isBox(m.plan)){ h+=rvPh('การแสดงผลบนแปลน'); h+=rvStyleTabRows(m);
...
function typePaletteRowHtml(m){
  ...
  if(t){
    ...
    h+='<div class="rv-pnote">'+rvIc('link',11)+' รูป/ข้อความ/ตาราง ในหน้ารายละเอียดของ “'+esc(t.name)+'” ใช้ร่วมกันทั้ง '+n+' ชิ้น — แก้ที่ชิ้นไหนก็เปลี่ยนทุกชิ้น</div>';
    h+='<div class="rv-pacts"><button class="btn soft" data-act="showDetails">'+rvIc('sheet',14)+' เปิดรายละเอียด</button><button class="btn soft" data-act="typeSelect" data-tid="'+esc(t.id)+'">'+rvIc('link',14)+' จัดการประเภท</button></div>';
  }else{
...
function typePaletteHtml(t){
  ...
  h+='<div class="rv-pnote">'+rvIc('link',11)+' แผ่นรายละเอียด '+typeDoc(t).els.length+' รายการ — ทุกชิ้นด้านล่างเห็นและแก้แผ่นเดียวกัน</div>';
  ...
  h+=rvPh('การจัดการ');
  h+='<div class="rv-pacts"><button class="btn" data-act="typeOpenDetail" data-tid="'+esc(t.id)+'"'+(ms.length?'':' disabled')+'>'+rvIc('sheet',14)+' เปิดรายละเอียด</button>'
...
function rvViewTabsHtml(f, plans){
  ...
  h+='<button class="rv-vt add" data-act="addPlan" title="นำเข้าแปลนใหม่">'+rvIc('plus',12)+'</button>';
...
function rvStatusHtml(vm, selM){
  ...
  if(!prog){
    ...
    if(_unp) h+='<button class="rv-sc warn" data-act="warnUnplaced" title="มีชิ้นส่วนที่เพิ่มด้วยฟอร์มแต่ยังไม่ได้วาดตำแหน่งบนแปลน — กดเพื่อดูรายชื่อ">'+rvIc('warn',12)+' <b>'+_unp+'</b> ยังไม่วาง</button>';
  }else{
  ...
  if(!prog) h+=_stBtn(state.autoCode,"toggleAutoCode","hash","เบอร์อัตโนมัติ","วาดต่อเนื่อง ใส่เบอร์ถัดไปให้เอง — แก้ทีหลังได้");
  ...
  h+='<span class="rv-ssep"></span><span class="rv-zoom"><button data-act="zoomOut" title="ซูมออก (−)">−</button><span id="zoomLabel">'+Math.round((state.zoom||1)*100)+'%</span><button data-act="zoomIn" title="ซูมเข้า (+)">+</button><button data-act="zoomFit" title="พอดีจอ (0)">'+rvIc('fit',12)+'</button></span>';
  return h+'</div>';
}
...
function areaFormatHtml(a){
  ...
  var h='<div class="fx" id="annotFormat"><div class="fx-h"><span>พื้นที่ที่วัด</span><button class="fx-x" data-act="annotClose" title="ปิด">'+rvIc('cross',12)+'</button></div><div class="fx-b">';
...
function tboxFormatHtml(a){
  ...
  var h='<div class="fx" id="annotFormat"><div class="fx-h"><span>รูปแบบ · '+(a.tip?'ป้ายชี้':'ป้ายข้อความ')+'</span><button class="fx-x" data-act="annotClose" title="ปิด">'+rvIc('cross',12)+'</button></div><div class="fx-b">';
  ...
  h+='<div class="fx-s">ลูกศรชี้</div>';
  if(a.tip){
    var AH=[["open","M3 4 11 8 3 12","none"],["solid","M3 4 11 8 3 12Z","currentColor"],["dot","",""],["none","",""]];
    h+='<div class="fx-r"><span class="fx-lb">หัวลูกศร</span><span style="display:inline-flex;gap:2px">'+AH.map(function(o){
      var ic=o[0]==="dot"?'<circle cx="11" cy="8" r="2.6" fill="currentColor"/>':o[0]==="none"?'':'<path d="'+o[1]+'" fill="'+o[2]+'" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/>';
      return '<button class="fx-tb'+(s.a1===o[0]?" on":"")+'" data-act="tboxArrow" data-v="'+o[0]+'" title="'+({open:"ลูกศรเปิด",solid:"ลูกศรทึบ",dot:"จุด",none:"ไม่มีหัว"})[o[0]]+'"><svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M1 8h10"/>'+ic+'</svg></button>'; }).join("")+'</span></div>';
    h+='<div class="fx-r"><button class="btn soft" style="flex:1;justify-content:center" data-act="tboxAddTip">เอาลูกศรออก</button></div>';
  }else h+='<div class="fx-r"><button class="btn soft" style="flex:1;justify-content:center" data-act="tboxAddTip">+ ใส่ลูกศรชี้</button></div>';
  h+='<div class="fx-note">ดับเบิลคลิกป้ายเพื่อแก้ข้อความ · ลากจุดมุม = ย่อ/ขยายทั้งป้าย · ลากจุดกลางขอบ = ปรับเฉพาะกล่อง'+(a.tip?' · ลากจุดขาวที่ปลายลูกศรเพื่อย้ายจุดชี้':'')+'</div>';
...
function mCardHtml(m){
  ...
  var st=memberStatus(m), ins=lastInspection(m.id);
  var stTx = st==="pass"?'<span class="ok">● ผ่าน พร้อมเท</span>' : st==="fail"?'<span class="bad">● ต้องแก้ไข</span>' : '<span class="wait">● รอตรวจ</span>';
...
function viewPlanEditor(){
  ...
  var hint='';
  if(state.tool==="drawTcall") hint='<div class="plan-tip">ป้ายชี้: คลิกจุดที่จะชี้ แล้วคลิกตรงที่จะวางป้าย (หรือลากจากจุดชี้ไปวาง) · Esc ยกเลิก</div>';
...
function mSheetWrap(id, title, body, opts, de){
  ...
  return (opts.dim?'<div class="m-dim" '+A+'="'+C+'"></div>':'')
    ...
    +(title?'<div class="m-sh">'+title+'<button class="m-x" '+A+'="'+C+'" title="ปิด">'+rvIc('cross',13)+'</button></div>':'')
...
function viewPlanEditorMobile(f,type,p,plan,plans,vm,selM,hint){
  ...
  var bar='<div class="m-bar">'
    ...
    + mBarBtn(state.tool==="drawCallout"||state.tool==="drawDim"||state.tool==="drawTbox"||state.tool==="drawTcall"||state.mSheet==="annot","mSheet",'data-sheet="annot"','mb_clip','หมายเหตุ')
...
function mPlanSheetHtml(f,type,p,plan,plans,selM,prog){
  ...
  if(sh==="zone"){
    ...
    var b3='<div class="m-grid">'+mG("drawZoneStart",'data-shape="rect"','rect','โซนสี่เหลี่ยม','',0,state.tool==="drawZone"&&(state.zoneShape||"rect")==="rect")+mG("drawZoneStart",'data-shape="poly"','poly','โซนหลายเหลี่ยม','',0,state.tool==="drawZone"&&state.zoneShape==="poly")+mG("drawZoneStart",'data-shape="oval"','oval','โซนวงรี','',0,state.tool==="drawZone"&&state.zoneShape==="oval")
      +mG("manageZoneStatus",'','gear','สถานะ + สี')+mG("exportProgressPdf",'','pdf','PDF เทคอนกรีต')+'</div>';
    ...
    b3+='<div class="m-acts"><button class="btn soft" data-act="mExitProgress">'+rvIc('back',12)+' กลับโหมดตรวจเหล็ก</button></div>';
  ...
  if(sh==="more"){
    ...
    b4+='<div class="m-sec">อื่น ๆ</div><div class="m-grid">'+mG("addInCat",'','plus','เพิ่มด้วยฟอร์ม')+mG("qatData",'','data','ข้อมูล/สำรอง')+mG("back",'','back','กลับหน้าชั้น')+'</div>';
  ...
  if(selM && !prog){
    ...
    if(insp){
      ...
      bm+='<div class="m-grid">'
        + mG("goInspect",'','check','ตรวจเหล็ก') + mG("showDetails",'','sheet','รายละเอียด') + mG("typeAssignSheet",'','link','ประเภท') + (isBox(selM.plan)?mG("mSheet",'data-sheet="style"','palette','สี / กรอบ'):mG("editMember",'','edit','แก้สเปก'))
        + mG("dupMember",'data-id="'+esc(selM.id)+'"','copy','ทำซ้ำ') + mG("toggleHide",'data-id="'+esc(selM.id)+'"',selM.hidden?'eyeOff':'eye',selM.hidden?'แสดง':'ซ่อน') + (isBox(selM.plan)?mG("editMember",'','edit','แก้สเปก'):mG("assignSheet",'','template','เทมเพลต')) + mG("delMember",'','trash','ลบ','danger')
        +'</div>';
      bm+='<div class="m-row"><span>ประเภท</span><select id="typeSel" class="m-in">'+typeSelOptionsHtml(selM)+'</select></div>';
      if(t) bm+='<div class="m-note">'+rvIc('link',11)+' หน้ารายละเอียดใช้ร่วมกัน '+typeMembers(t.id).length+' ชิ้น — แก้ที่ชิ้นไหนก็เปลี่ยนทุกชิ้น</div>';
...
function viewMemberDetailMobile(m,p,f,t,sib){
  ...
  var top=mTopHtml('รายละเอียด '+esc(m.code), (t?''+rvIc('link',11)+' '+esc(t.name)+' · '+typeMembers(t.id).length+' ชิ้น':esc(TYPES[m.type].label))+' · '+esc(f?f.name:''),
  ...
  var floats='<div class="m-fl"><button class="m-fb" data-de="undo" title="ย้อนกลับ"'+(canU?'':' disabled')+'>'+rvIc('undo',18)+'</button><button class="m-fb" data-de="redo" title="ทำซ้ำ"'+(canR?'':' disabled')+'>'+rvIc('redo',18)+'</button><button class="m-fb" data-de="zoomfit" title="พอดีจอ">'+rvIc('fit',18)+'</button></div>'
    ...
    +'<button class="m-fab" data-de="msheet" data-sheet="add" title="เพิ่ม">'+rvIc('plus',26)+'</button>';
...
function mDetailSheetHtml(m,t,sib){
  ...
  if(sh==="add"){
    var b='<div class="m-grid">'+mGd("cam",'','camera','ถ่ายรูป')+mGd("file",'','img','เพิ่มไฟล์')+mGd("addtext",'','text','ข้อความ')+mGd("addtable",'','grid','ตาราง')
      +mGd("msheet",'data-sheet="lib"','shapes','รูปทรง / เส้น')+mGd("stamp",'data-sub="recheck"','stamp','รอตรวจซ้ำ')+mGd("stamp",'data-sub="name"','user','ชื่อผู้ตรวจ')+mGd("stamp",'data-sub="date"','calendar','วันที่วันนี้')+'</div>'
  ...
  if(sh==="annot"){
    ...
    var b2='<div class="m-grid">'+mGd("tool",'data-tool="callout"','callout','กล่องข้อความ','',0,tl==="callout")+mGd("tool",'data-tool="dim"','dim','เส้นบอกขนาด','',0,tl==="dim")+mGd("tool",'data-tool="pen"','pen','ปากกา','',0,tl==="pen")+mGd("tool",'data-tool="hilite"','hilite','ไฮไลต์','',0,tl==="hilite")
      +mGd("stamp",'data-sub="pass"','check','ตราผ่าน','ok')+mGd("stamp",'data-sub="fix"','cross','ตราแก้ไข','bad')+mGd("stamp",'data-sub="recheck"','stamp','รอตรวจซ้ำ')+mGd("tool",'data-tool="select"','sel','เลือก/ย้าย','',0,tl==="select")+'</div>'
  ...
  if(sh==="more"){
    ...
    b4+='<div class="m-sec">ชิ้นส่วนนี้</div><div class="m-grid">'+mGd("inspect",'','check','ตรวจเหล็ก')+mGd("exportpdf",'','pdf','ออก PDF')
      +(t?mG("typeManage",'','link','จัดการประเภท')+mG("typeDetach",'','unlink','แยกออก'):mG("back",'','back','กลับหน้าแปลน'))+'</div>';
    if(t) b4+='<div class="m-note">'+rvIc('link',11)+' '+esc(t.name)+' · ใช้ร่วมกัน '+typeMembers(t.id).length+' ชิ้น — แก้ที่นี่ = เปลี่ยนทุกชิ้น</div>';
    b4+='<div class="m-grid" style="margin-top:8px">'+mG("saveNow",'','save','บันทึก')+mG("back",'','back','กลับหน้าแปลน')+'</div>';
...
function bindPlanEditor(){
  ...
  _plKeyHandler=function(e){
    ...
    var prog=stageMode()==="progress";
    if(kl==="c" && !prog){ state.tool="drawTcall"; state.selAnnotId=null; state.selMemberId=null; state.selMulti=[]; state.marquee=false; render(); }
  ...
  function applyStyle(colOverride, which){
    var _mp=(function(){ var q=getMember(state.selMemberId); return (q&&isBox(q.plan))?q.plan:null; })();
    var col=colOverride||(dc?dc.value:(_mp?(_mp.fill||state.fillColor):state.fillColor));
    var a=da?(+da.value)/100:(_mp&&_mp.fillA!=null?_mp.fillA:state.fillAlpha);
    var ww=ds?+ds.value:(_mp&&_mp.strokeW!=null?_mp.strokeW:state.strokeW);
    ...
    if(_multi && _multi.length){   // เลือกหลายกล่อง → ใช้ค่าที่กดกับทุกกล่องที่เลือก
      ...
      _multi.forEach(function(s){
        ...
        if(which!=="stroke") setGroupFill(s, s.plan.fill, s.plan.fillA);   // เบอร์/ประเภทเดียวกันตามด้วย
        if(!which||which==="stroke") setGroupStroke(s, ww);
    ...
    if(mm && isBox(mm.plan)){    // มีคานเลือกอยู่ → ปรับคานนั้น
      ...
      setGroupFill(mm, col, a);   // เบอร์/ประเภทเดียวกัน = สี + ความเข้มเดียวกันเสมอ (แก้ชิ้นที่ค้างค่าผิดจากข้อมูลเก่าด้วย)
      setGroupStroke(mm, ww);     // เส้นกรอบเท่ากันทั้งกลุ่มด้วย
      var r=$("#planOverlay").querySelector('[data-mid="'+mm.id+'"]');   // rect/ellipse/polygon
      if(r){
        r.setAttribute("fill",col); r.setAttribute("fill-opacity",a);
        if(ww>0){ r.setAttribute("stroke",mm.plan.strokeCol||col); r.setAttribute("stroke-width",ww); } else r.setAttribute("stroke","none");
      }
      ...
      var _doF=(!which||which==="fill"), _doA=(!which||which==="alpha"), _doW=(!which||which==="stroke");
      var _f=getFloor(state.floorId), _pid=curPlanId(), _n=0, _gSeen={};
      membersOfFloor(_f.id).forEach(function(mm){ if(mm.plan && isBox(mm.plan) && memberPlanId(mm)===_pid){
        if(_doF) mm.plan.fill=col; if(_doA) mm.plan.fillA=a; if(_doW) mm.plan.strokeW=ww; _n++;
        var _gk=mm.typeId?("t:"+mm.typeId):("c:"+_codeKey(mm)); if(!_gSeen[_gk]) _gSeen[_gk]=mm;
      } });
      // เบอร์/ประเภทเดียวกันในชั้นอื่นตามด้วย (ไม่งั้นเปิดแอปใหม่สีไม่ตรงกัน)
      if(_doF||_doA) Object.keys(_gSeen).forEach(function(gk){ setGroupFill(_gSeen[gk], _doF?col:null, _doA?a:null); });
      if(_doW) Object.keys(_gSeen).forEach(function(gk){ setGroupStroke(_gSeen[gk], ww); });
      if(_n) repaintPlanShapes();
    }
  }
  function saveStyle(which){ applyStyle(null, which); saveDB(); }
  ...
  if(dc){ dc.addEventListener("input",function(){ applyStyle(null,"fill"); }); dc.addEventListener("change",function(){ saveStyle("fill"); }); }
  if(da){ da.addEventListener("input",function(){ applyStyle(null,"alpha"); }); da.addEventListener("change",function(){ saveStyle("alpha"); }); }
  if(ds){ ds.addEventListener("input",function(){ applyStyle(null,"stroke"); }); ds.addEventListener("change",function(){ saveStyle("stroke"); }); }
  ...
  $$("[data-qalpha]").forEach(function(b){ b.addEventListener("click",function(){
    ...
    applyStyle(null,"alpha"); saveDB(); render();
  }); });
  $$("[data-qstroke]").forEach(function(b){ b.addEventListener("click",function(){
    ...
    applyStyle(null,"stroke"); saveDB(); render();
  }); });
  ...
  function _applyStrokeCol(v){
    ...
    list.forEach(function(mm2){
      setGroupStroke(mm2, undefined, v||"");   // เบอร์/ประเภทเดียวกันตามด้วย
  ...
  if(dsc){
    dsc.addEventListener("pointerdown",function(){ plPushUndo(); });
    dsc.addEventListener("input",function(){ _applyStrokeCol(dsc.value); });
    dsc.addEventListener("change",function(){ saveDB(); render(); });
  }
  // ช่องตัวเลข ↔ สไลเดอร์ (%, px)
  function _pairNum(sl,nm,which){
    if(!sl||!nm) return;
    sl.addEventListener("input",function(){ nm.value=sl.value; });
    nm.addEventListener("change",function(){ plPushUndo(); sl.value=nm.value; applyStyle(null,which); saveDB(); });
  }
  _pairNum(da,$("#alphaNum"),"alpha"); _pairNum(ds,$("#strokeNum"),"stroke");
  ...
  if(state.tool==="select"){
    ...
    overlay.addEventListener("contextmenu",function(ev){
      ...
      ev.preventDefault();
      var _inSet=selIds().indexOf(cm.id)>=0;
      if(state.selMemberId!==cm.id || !_inSet){ if(!_inSet) state.selMulti=[]; state.selMemberId=cm.id; state.memberId=cm.id; state.selZoneId=null; state.selAnnotId=null; state.selTypeId=null; if(state.rightTab!=="inspect") state.rightTab="props"; _toModifyTab(); render(); }
    ...
    overlay.addEventListener("dblclick",function(ev){
      ...
      state.selMemberId=dm.id; state.memberId=dm.id; state.rpCollapsed=false; _toModifyTab();
    ...
    function selMove(ev){
      ...
      if(atf){       // ลากจุดจับหมายเหตุ
        ...
        }else if(at.kind==="tbox" && hh2==="tip"){ at.tip={x:cx2,y:cy2};
    ...
    function selUp(ev){
      ...
      if(mq){        // ปล่อยกรอบเลือก → เลือกทุกชิ้นที่กรอบทับ
        ...
        setMulti(ids); state.marquee=false; if(ids.length && !isMobile()) _toModifyTab(); if(state.rightTab!=="inspect") state.rightTab="props";
  ...
  if(state.tool==="drawCallout" || state.tool==="drawDim" || state.tool==="drawTbox" || state.tool==="drawTcall"){
    var isDimTool=(state.tool==="drawDim"), isTbox=(state.tool==="drawTbox"), isTcall=(state.tool==="drawTcall");
    var ACOL=(isTbox||isTcall) ? tboxStyleOf(null).bcol : ((state.annotStyle&&state.annotStyle.color)||"#1d4ed8");
    var aStart=null, aTemp=null, tcTip=null, tcTemp=null;
    function tcClear(){ if(tcTemp){ tcTemp.remove(); tcTemp=null; } tcTip=null; }
    function aEnd(ev2){
      ...
      if(isTcall){                                            // ป้ายชี้: คลิกแรก = จุดชี้ · คลิกสอง = ที่วางป้าย
        if(tcTip){ var _tt=tcTip; tcClear(); finishDrawTcall(_tt,p); return; }
        if(!_short){ finishDrawTcall(s0,p); return; }        // ลากจากจุดชี้ไปวางป้ายในครั้งเดียว
        tcTip=s0; var _z0=state.zoom||1;
        tcTemp=document.createElementNS(NS,"g");
        tcTemp.innerHTML='<line x1="'+(s0.x*VW)+'" y1="'+(s0.y*VH)+'" x2="'+(s0.x*VW)+'" y2="'+(s0.y*VH)+'" stroke="'+ACOL+'" stroke-width="'+(2/_z0)+'" stroke-dasharray="'+(6/_z0)+' '+(4/_z0)+'"/>'
          +'<circle cx="'+(s0.x*VW)+'" cy="'+(s0.y*VH)+'" r="'+(4/_z0)+'" fill="'+ACOL+'"/>';
        tcTemp.style.pointerEvents="none"; overlay.appendChild(tcTemp);
        return;
      }
    ...
    overlay.addEventListener("pointerdown",function(ev){
      ...
      aStart=snapAt(ev);
      var z=state.zoom||1;
      if(isTcall && tcTip){ aTemp=null;   // คลิกที่สองของป้ายชี้ — ไม่ต้องมีกรอบชั่วคราว
        window.addEventListener("pointermove",aMove,true); window.addEventListener("pointerup",aEnd,true); window.addEventListener("pointercancel",aEnd,true);
        return; }
      aTemp=document.createElementNS(NS, (isDimTool||isTcall)?"line":"rect");
      aTemp.setAttribute("stroke",ACOL); aTemp.setAttribute("stroke-width",(2/z));
      aTemp.setAttribute("stroke-dasharray",(6/z)+" "+(4/z));
      if(isDimTool||isTcall){ aTemp.setAttribute("x1",aStart.x*VW); aTemp.setAttribute("y1",aStart.y*VH); aTemp.setAttribute("x2",aStart.x*VW); aTemp.setAttribute("y2",aStart.y*VH); }
...
/* ---- กลุ่มสี: ชนิด + เบอร์เดียวกันในโครงการ (รวมทุกชิ้นของประเภทที่เกี่ยวข้อง) ---- */
function _codeKey(m){ return m.projectId+"|"+m.type+"|"+String(m.code||"").trim(); }
function colorGroupOf(m){
  if(!m) return [];
  var out=[], tids={}, hasCode=!!String(m.code||"").trim(), k=hasCode?_codeKey(m):null;
  if(m.typeId) tids[m.typeId]=1;
  (DB.members||[]).forEach(function(x){
    if(x.id===m.id || (hasCode && _codeKey(x)===k)){ out.push(x); if(x.typeId) tids[x.typeId]=1; }
  });
  Object.keys(tids).forEach(function(tid){ typeMembers(tid).forEach(function(x){ if(out.indexOf(x)<0) out.push(x); }); });
  return out;
}
/** ตั้งสี/ความเข้มให้ทั้งกลุ่ม + ประเภทที่เกี่ยวข้อง แล้ววาดใหม่ทันที · null = ไม่แตะ */
function setGroupFill(m, col, fa){
  if(!m || (!col && fa==null)) return;
  var grp=colorGroupOf(m), ts={};
  grp.forEach(function(s){ if(s.typeId) ts[s.typeId]=1; });
  Object.keys(ts).forEach(function(tid){ var t=getType(tid); if(t) setTypeFill(t, col, fa); });
  var ov=document.getElementById("planOverlay");
  grp.forEach(function(s){
    ...
    var r=ov && ov.querySelector('[data-mid="'+s.id+'"]');
    if(r && state.colorMode!=="status"){
      if(col){ r.setAttribute("fill",col); var w=(s.plan.strokeW!=null)?s.plan.strokeW:10; if(w>0) r.setAttribute("stroke", s.plan.strokeCol||col); }
      if(fa!=null) r.setAttribute("fill-opacity",fa);
    }
  });
}
/** เส้นกรอบทั้งกลุ่ม: w = ความหนา (undefined = ไม่แตะ) · sc = สีเส้น (undefined = ไม่แตะ, "" = ตามสีพื้น) */
function setGroupStroke(m, w, sc){
  if(!m || (w==null && sc===undefined)) return;
  var ov=document.getElementById("planOverlay");
  colorGroupOf(m).forEach(function(s){
    if(!s.plan || !isBox(s.plan)) return;
    if(w!=null) s.plan.strokeW=w;
    if(sc!==undefined){ if(sc) s.plan.strokeCol=sc; else delete s.plan.strokeCol; }
    var r=ov && ov.querySelector('[data-mid="'+s.id+'"]');
    if(r && state.colorMode!=="status"){
      var ww=(s.plan.strokeW!=null)?s.plan.strokeW:10;
      if(ww>0){ r.setAttribute("stroke", s.plan.strokeCol||s.plan.fill||state.fillColor); r.setAttribute("stroke-width", ww); } else r.setAttribute("stroke","none");
    }
  });
}
/** ชิ้นที่เพิ่งวาด/เปลี่ยนเบอร์ → ใช้สี ความเข้ม และเส้นกรอบเดียวกับกลุ่มเดิม (ถ้ามี) */
function adoptGroupStyle(m){
  if(!m || !m.plan || !isBox(m.plan)) return;
  var others=colorGroupOf(m).filter(function(x){ return x.id!==m.id && x.plan && isBox(x.plan); });
  var ref=others.filter(function(x){ return x.typeId && getType(x.typeId); })[0] || others[0];
  if(ref){
    ["fill","fillA","strokeW","strokeCol"].forEach(function(k){ if(ref.plan[k]!=null) m.plan[k]=ref.plan[k]; else delete m.plan[k]; });
  }
  var t=m.typeId?getType(m.typeId):null;
  if(t) syncMemberToType(m, t);
}
...
function finishDraw(geom){
  ...
  if(state.autoCode){ code=suggest; }          // วาดต่อเนื่อง: ไม่ถาม ใช้เบอร์ถัดไป
    ...
    code=window.prompt("ใส่เบอร์"+TYPES[type].label+" (เช่น "+suggest+"):", suggest);
    if(code===null){ toast("ยกเลิกการวาด — กรอบไม่ถูกบันทึก"); render(); return; }        // ยกเลิก (ยังไม่ push undo)
  ...
  if(unplaced){
    ...
    adoptGroupStyle(unplaced);
    if(!saveDB()){ delete unplaced.plan; delete unplaced.planId; render(); return; }
    state.selMemberId=unplaced.id; if(!isMobile()) _toModifyTab();
  ...
  if(twin){
    ...
    adoptGroupStyle(m);   // สี/ความเข้ม/เส้นกรอบ = เบอร์เดิม (ไม่ใช้สีที่ตั้งไว้สำหรับวาดใหม่)
    DB.members.push(m);
  ...
  m.planId=curPlanId();
  adoptGroupStyle(m);   // เบอร์นี้มีในชั้นอื่นแล้ว → ใช้สีเดียวกัน
  DB.members.push(m);
  if(!saveDB()){ DB.members.pop(); render(); return; }
...
/** ป้ายชี้: tip = จุดที่ชี้ · p = ที่วางป้าย (ขอบกล่องฝั่งใกล้ลูกศรอยู่ตรงจุดที่คลิก) */
function finishDrawTcall(tip, p){
  var ov=document.getElementById("planOverlay");
  var VW=(ov&&+ov.getAttribute("data-vw"))||1000, VH=(ov&&+ov.getAttribute("data-vh"))||1000;
  var st=tboxStyleOf(null), w=0.16, h=Math.max(0.02, st.size*(VW/1000)*2.3/VH);
  var x1=(p.x>=tip.x)?p.x:p.x-w; x1=Math.max(0,Math.min(1-w,x1));
  var y1=Math.max(0,Math.min(1-h,p.y-h/2));
  var a={ id:uid("an"), projectId:state.projectId, floorId:state.floorId, planId:curPlanId(), kind:"tbox",
          box:{x1:x1,y1:y1,x2:x1+w,y2:y1+h}, tip:{x:tip.x,y:tip.y}, text:"ข้อความ" };
...
function finishSetScale(s,p){
  ...
  if(v===null){ toast("ยกเลิก — ยังไม่ได้ตั้งมาตราส่วน"); state.tool="select"; render(); return; }
...
function finishDrawArea(pts){
  ...
  if(name===null){ toast("ยกเลิก — ไม่ได้บันทึกพื้นที่"); state.tool="select"; render(); return; }
  name=name.trim()||sug;
...
function areaToZone(a){
  ...
  toast("สร้างโซนเท “"+z.name+"” จากพื้นที่นี้แล้ว — ดูในแท็บเทคอนกรีต");
}
...
function finishDrawZone(geom){
  ...
  if(name===null){ toast("ยกเลิก — ไม่ได้บันทึกโซน"); state.tool="select"; render(); return; }
...
function normalizeMemberTypes(){
  ...
  // เบอร์เดียวกันที่ไม่ผูกประเภท → สี/ความเข้มตามกลุ่ม · อ้างอิง = ประเภทของเบอร์นั้น (ถ้ามี) ไม่งั้นชิ้นที่สร้างก่อนสุด (id น้อยสุด) → ทุกเครื่องได้ผลเดียวกัน
  var groups={};
  (DB.members||[]).forEach(function(m){
    if(!m || !m.plan || !isBox(m.plan) || !String(m.code||"").trim()) return;
    var k=_codeKey(m); (groups[k]=groups[k]||[]).push(m);
  });
  var byId=function(a,b){ return a.id<b.id?-1:(a.id>b.id?1:0); };
  Object.keys(groups).forEach(function(k){
    var g=groups[k]; if(g.length<2) return;
    var typed=g.filter(function(x){ return x.typeId && getType(x.typeId); }).sort(function(a,b){ return a.typeId<b.typeId?-1:(a.typeId>b.typeId?1:byId(a,b)); })[0];
    var rf, ra, rsrc;
    if(typed){ var tt=getType(typed.typeId); rf=tt.color||typed.plan.fill; ra=(tt.fillA!=null)?tt.fillA:typed.plan.fillA; rsrc=typed; }
    else { var r0=g.slice().sort(byId)[0]; rf=r0.plan.fill; ra=r0.plan.fillA; rsrc=r0; }
    var rw=rsrc.plan.strokeW, rc=rsrc.plan.strokeCol;
    g.forEach(function(x){   // เส้นกรอบ (ความหนา/สีเส้น) — ทุกชิ้นในกลุ่ม รวมชิ้นที่ผูกประเภท
      if(x===rsrc) return;
      if(rw!=null){ if(x.plan.strokeW!==rw){ x.plan.strokeW=rw; changed=true; } } else if(x.plan.strokeW!=null){ delete x.plan.strokeW; changed=true; }
      if(rc){ if(x.plan.strokeCol!==rc){ x.plan.strokeCol=rc; changed=true; } } else if(x.plan.strokeCol){ delete x.plan.strokeCol; changed=true; }
    });
    g.forEach(function(x){
      if(x.typeId && getType(x.typeId)) return;   // ชิ้นที่ผูกประเภทตามประเภทของตัวเอง (ด้านบน)
      if(rf!=null){ if(String(x.plan.fill||"").toLowerCase()!==String(rf).toLowerCase()){ x.plan.fill=rf; changed=true; } }
      else if(x.plan.fill!=null){ delete x.plan.fill; changed=true; }
      if(ra!=null){ if(x.plan.fillA!==ra){ x.plan.fillA=ra; changed=true; } }
      else if(x.plan.fillA!=null){ delete x.plan.fillA; changed=true; }
    });
  });
  if(changed) saveDB();
}
