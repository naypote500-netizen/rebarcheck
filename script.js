...
function bindPlanEditor(){
  ...
  function applyStyle(colOverride, which){
    ...
    if(mm && isBox(mm.plan)){    // มีคานเลือกอยู่ → ปรับคานนั้น
      ...
      var _f=getFloor(state.floorId), _pid=curPlanId(), _n=0, _tSeen={};
      membersOfFloor(_f.id).forEach(function(mm){ if(mm.plan && isBox(mm.plan) && memberPlanId(mm)===_pid){
        ...
        if(mm.typeId) _tSeen[mm.typeId]=1;
      } });
      // กล่องที่ผูกประเภท: ตั้งค่าที่ประเภทด้วย (ไม่งั้นเปิดแอปใหม่เด้งกลับ) — ชิ้นประเภทเดียวกันชั้นอื่นตามด้วย
      if(_doF||_doA) Object.keys(_tSeen).forEach(function(tid){ var _t=getType(tid); if(_t) setTypeFill(_t, _doF?col:null, _doA?a:null); });
